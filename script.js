...
async function handleSignout() {
  // Always force-clear everything first so UI never gets stuck
  currentUser = null;
  closeMobileMenu();
  ...
  document.getElementById('auth-error').textContent     = '';
  // Wipe all Supabase cached tokens from localStorage
  Object.keys(localStorage).forEach(key => {
    if (key.startsWith('sb-')) localStorage.removeItem(key);
  });
  // Then tell Supabase to sign out (fire and forget — UI already cleared)
  try { await db.auth.signOut({ scope: 'local' }); } catch (e) { console.warn(e); }
}
  ...
  try {
    ...
    if (pendingPhotos.length > 0) {
      ...
      for (const { file } of pendingPhotos) {
        ...
        const { error: upErr } = await db.storage.from('photos').upload(path, file, { contentType: file.type });
        if (upErr) { console.error('Photo upload error:', upErr.message, upErr); photoErrors++; continue; }
        const { data: urlData } = db.storage.from('photos').getPublicUrl(path);
        const { error: dbErr } = await db.from('photos').insert({ workout_id: workoutId, storage_path: path, public_url: urlData.publicUrl });
        if (dbErr) console.error('Photo DB insert error:', dbErr.message);
      }
      if (photoErrors > 0) showToast(`${photoErrors} photo(s) failed to upload. Check storage bucket is public.`, true);
    }
